...
namespace prism
{
	...
	Iterator* Table::BlockReaderByHandle(Table* table, const ReadOptions& options, const BlockHandle& handle)
	{
	...
	Iterator* Table::BlockReader(void* arg, const ReadOptions& options, const Slice& index_value)
	{
		...
		return table->BlockReaderByHandle(table, options, handle);
	}

	Status Table::InternalGet(const ReadOptions& options, const Slice& key, void* arg, HandleResult handle_result)
	{
		Status s;
		BlockHandle handle;
		...
		if (index_iter->Valid())
		{
			...
			if (s.ok())
			{
				...
				if (filter != nullptr && !filter->KeyMayMatch(handle.offset(), key))
				{
			...
			{
				Iterator* block_iter = BlockReaderByHandle(this, options, handle);
				block_iter->Seek(key);
				if (block_iter->Valid())
				{
