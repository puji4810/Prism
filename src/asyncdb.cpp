...
namespace prism
{
	...
	AsyncOp<Result<AsyncDB>> AsyncDB ::OpenAsync(ThreadPoolScheduler& scheduler, const Options& options, std::string dbname)
	{
		...
		return AsyncOp<Result<AsyncDB>>(
		    RuntimeFrom(runtime).foreground_db_scheduler, [runtime, options, dbname = std::move(dbname)]() -> Result<AsyncDB> {
	...
	AsyncOp<Result<std::string>> AsyncDB::GetAsync(const ReadOptions& options, std::string key)
	{
		return AsyncOp<Result<std::string>>(
		    RuntimeFrom(state_->runtime_).foreground_db_scheduler, [state = state_, opts = options, key = std::move(key)]() {
	...
	{
		return AsyncOp<Status>(RuntimeFrom(state_->runtime_).foreground_db_scheduler,
		    [state = state_, opts = options, batch = std::move(batch)]() mutable { return state->db_.Write(opts, std::move(batch)); });
	}
